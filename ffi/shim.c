...
static void ts_query_cursor_finalizer(void *ptr) {
    ...
    if (bc->tree_ref) lean_dec(bc->tree_ref);
    free(bc);
}

static lean_external_class *get_ts_query_cursor_class(void) {
    if (!g_ts_query_cursor_class)
        g_ts_query_cursor_class = lean_register_external_class(&ts_query_cursor_finalizer, &noop_foreach);
...
static void noop_finalizer(void *ptr) { (void)ptr; }
...
static void ts_node_finalizer(void *ptr) {
    ...
    if (boxed->tree_ref) lean_dec(boxed->tree_ref);
    free(boxed);
}
...
LEAN_EXPORT lean_obj_res lean_ts_parser_set_language(
    b_lean_obj_arg parser, b_lean_obj_arg lang, lean_obj_arg world) {
...
LEAN_EXPORT lean_obj_res lean_ts_node_start_byte(b_lean_obj_arg node, lean_obj_arg world) {
    TSNode n = unbox_ts_node(node);
...
LEAN_EXPORT lean_obj_res lean_ts_node_child_by_field_name(
    b_lean_obj_arg node, b_lean_obj_arg field_name, lean_obj_arg world) {
    TSNode n = unbox_ts_node(node);
...
LEAN_EXPORT lean_obj_res lean_ts_query_new(
    ...
    if (!q) {
        char buf[256];
        snprintf(buf, sizeof(buf), "Query error at offset %u (type %d)",
...
LEAN_EXPORT lean_obj_res lean_ts_query_cursor_exec(
    b_lean_obj_arg cursor, b_lean_obj_arg query, b_lean_obj_arg node, lean_obj_arg world) {
    ...
    if (bc->tree_ref) lean_dec(bc->tree_ref);
    bc->tree_ref = tref;
    if (tref) lean_inc(tref);
    ts_query_cursor_exec(bc->cursor, q, n);
    return lean_io_result_mk_ok(lean_box(0));
}

LEAN_EXPORT lean_obj_res lean_ts_query_cursor_next_match(
    b_lean_obj_arg cursor, lean_obj_arg world) {
    BoxedTSQueryCursor *bc = (BoxedTSQueryCursor *)lean_get_external_data(cursor);
    TSQueryMatch match;
...
LEAN_EXPORT lean_obj_res lean_ts_query_capture_name(
    b_lean_obj_arg query, uint32_t index, lean_obj_arg world) {
    TSQuery *q = (TSQuery *)lean_get_external_data(query);
    uint32_t length;
